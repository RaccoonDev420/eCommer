...
public class BikeTypeSpecificationBuilder {

    private final List<SearchCriteria> params;

    public BikeTypeSpecificationBuilder() {
        params = new ArrayList<>();
    }

    public BikeTypeSpecificationBuilder with(String key, String operation, Object value,String isOr){
        params.add(new SearchCriteria(key,operation,value,isOr));
        return this;
    }

    public BikeTypeSpecificationBuilder with(String key, String operation, Object value){
        params.add(new SearchCriteria(key,operation,value));
        return this;
    }

    public Specification<BikeType> build(){
        if(params.size() == 0){
            return null;
        }

        List<Specification> specs = params.stream().map(BikeTypeSpecification::new).collect(Collectors.toList());
